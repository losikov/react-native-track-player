...
class PlayerEventHolder {
    ...
    private var _progressDiscontinuity = MutableSharedFlow<Unit>(0)

    /**
     * A position jump that no interval tick will report: a seek (including a seek while paused) and
     * an atomic queue load. `MusicService` turns each one into a single progress event, so JS's
     * stored progress is not left holding the pre-seek position until playback resumes.
     */
    var progressDiscontinuity = _progressDiscontinuity.asSharedFlow()

    private var _stopAtReached = MutableSharedFlow<Long>(0)

    /**
     * Playback reached the position armed by
     * [com.doublesymmetry.kotlinaudio.players.BaseAudioPlayer.setStopAt]; the engine has already
     * paused at it with [com.doublesymmetry.kotlinaudio.models.TransportReason.STOP_AT].
     */
    var stopAtReached = _stopAtReached.asSharedFlow()
    ...
    */
    ...
    internal fun updateProgressDiscontinuity() {
        coroutineScope.launch {
            _progressDiscontinuity.emit(Unit)
        }
    }

    internal fun updateStopAtReached(positionMs: Long) {
        coroutineScope.launch {
            _stopAtReached.emit(positionMs)
        }
    }
